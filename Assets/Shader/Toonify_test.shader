    ...
    SubShader
    {
        ...
        fixed3 BilateralFliter(half2 uv){
                ...
                for(int i = 0; i < 25; i++){
                    ...
                    fixed3 temp_color = tex2D(_MainTex, tex5[i]).rgb;
                    fixed final_weight = exp(-0.5 * sqrt(abs((tex5[i].x - uv.x) + (tex5[i].y - uv.y)) / 1)) * exp(-0.5 * sqrt(1 - abs(centerLum - lumiance(temp_color)) / 0.06));
                    res += temp_color * final_weight;
                ...
                */
                ...
                for(int i = 0; i < 9; i++){
                    ...
                    fixed final_weight = exp(-0.5 * sqrt(abs((tex9[i].x - uv.x) + (tex9[i].y - uv.y)) / 0.5)) * exp(-0.5 * sqrt(1 - abs(centerLum - lumiance(temp_color)) / 0.035));
                    //fixed final_weight = weight[i] * (1 - abs(centerLum - lumiance(temp_color)));
                    res += temp_color * final_weight;
                }
        ...
        fixed4 fragEdges(v2f f) : SV_Target{
            ...
            fixed3 gaussian_color = GaussianFliter(f.uv);
            ...
            fixed3 Toonify_color = ColorCalculation(color);


            fixed4 withEdgeColor = lerp(edgeColor, fixed4(temp_color, 1), edge);
            fixed4 onlyEdgeColor = lerp(edgeColor, backgroundColor, edge);

            //return onlyEdgeColor;
            //return fixed4(gaussian_color, 1);

            return lerp(withEdgeColor, onlyEdgeColor, 0.5);
